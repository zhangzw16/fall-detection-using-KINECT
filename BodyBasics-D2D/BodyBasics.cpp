...
static char* filePath = "C:\\Users\\ÕÅÓî¶«\\Desktop\\fall-detection-using-KINECT-master\\BodyBasics-D2D\\x64\\Debug\\";
                ...
                if (pBody)
                {
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            ...
                            {
								// !!Fall Detect here!!
								FallDetect(numFrames, joints, dataFile);
...
void CBodyBasics::FallDetect(const INT64 numFrames, const Joint* pJoints, std::ofstream& dataFile)
{
	if (numFrames % 3 == 0) {
